(function () {
  ...
  // purchases anything, or changes Discord billing. It only intercepts the
  // native Gift Inventory API response on this device.
  var FAKE_GIFTS = [
    ["fakegift_nitro_monthly", "Nitro · 1 Month", "511651880837840896", 1, 1],
    ["fakegift_nitro_3months", "Nitro · 3 Months", "BTLOCALPLAN000003", 3, 3],
    ["fakegift_nitro_6months", "Nitro · 6 Months", "BTLOCALPLAN000006", 6, 6],
    ["fakegift_nitro_yearly", "Nitro · 12 Months", "511651885459963904", 1, 12]
  ];
  ...
  var giftFetchOriginal = null;
  var giftFetchPatched = false;
  ...
  function clearFakeGiftPatches() {
    if (giftFetchPatched) {
      try {
        var root = globalThis;
        if (root && giftFetchOriginal) root.fetch = giftFetchOriginal;
      } catch (_) {}
    }
    giftFetchOriginal = null;
    giftFetchPatched = false;
  }
      ...
      FAKE_GIFTS.map(function (item) {
        return React.createElement(FormSwitchRow, {
          ...
          onValueChange: function (v) {
            ...
            clearFakeGiftPatches();
            setTimeout(function () { safe(patchFakeGiftInventory); }, 0);
            refresh();
          }
        });
      })
